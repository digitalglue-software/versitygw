...
func New(rootdir string, permStrategy string) (*Posix, error) {
	...
	fmt.Println("from construct permStrategy: ", permStrategy)

	return &Posix{rootfd: f, rootdir: rootdir, permStrategy: permStrategy}, nil
}
...
func (p *Posix) ListBuckets(_ context.Context, owner string, isAdmin bool) (s3response.ListAllMyBucketsResult, error) {
	...
	for _, entry := range entries {
		...
		} else if p.permStrategy == "posix" {
			access, err := AccessCheck(owner, []string{}, filepath.Join(p.rootdir, entry.Name()), "r")
			if err != nil {
				return s3response.ListAllMyBucketsResult{}, fmt.Errorf("posix permission check: %w", err)
			}
